...
#+end_src
*** make it loop
To create a loop in your shell script, you can use a =while= loop. Here's a modified version of your code that loops continuously:
...
This loop will repeatedly run the unit tests until you manually exit the script.
*** add pause on bash script
To add a pause in a Bash script, you can use the =read= command. Modify your script like this:
...
This will pause the script execution until a key is pressed.
